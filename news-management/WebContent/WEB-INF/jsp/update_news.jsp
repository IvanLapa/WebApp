...
<fmt:message bundle="${loc}" key="button.update" var="button_update" />

<fmt:message bundle="${loc}" key="operation.update-news.logo" var="operation_update_news_logo" />

<body>
	<p align="right">
		<a href="controller?command=localization&local=en&current_command=localization_update_news_command">${button_en}</a> 
		<a href="controller?command=localization&local=ru&current_command=localization_update_news_command">${button_ru}</a>
	</p>
	...
	<form action="controller" method="POST">
		...
		<input type="hidden" name="id" value="${sessionScope.current_news.id}" />
		<input type="hidden" name="date" value="${sessionScope.current_news.date}" />
		<fieldset>
		<legend>${operation_update_news_logo}</legend>
		<p>
			<label for="name">ID:</label>
			<c:out value="${sessionScope.current_news.id}" />
		</p>
		<p>
			<label for="name">${form_title}</label>
			<br>
			<input type="text"  name="title" value="${sessionScope.current_news.title}" required>
		</p>
		<p>						
			<label for="name">${form_brief}</label>
			<br>
			<input type="text" name="brief" value="${sessionScope.current_news.brief}" required>
		</p>
		<p>	
			<label for="name">${form_content}</label>
			<br>
			<textarea name="content" required><c:out value="${sessionScope.current_news.content}" /></textarea>
		</p>
			<input type="submit" name="submit" value = "${button_update}" class="myButton">
			<br>
		</fieldset>
		<br>
